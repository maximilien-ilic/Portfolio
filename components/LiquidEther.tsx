...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const bgVec4 = new THREE.Vector4(0.99, 0.55, 0.01, 0.3); // Légère couleur de base orange
...
`;
    ...
    void main(){
    vec2 vel = texture2D(velocity, uv).xy;
    float lenv = clamp(length(vel) * 2.0, 0.0, 1.0);
    vec3 c = texture2D(palette, vec2(lenv, 0.5)).rgb;
    float mixAmount = smoothstep(0.0, 0.3, lenv);
    vec3 outRGB = mix(bgColor.rgb, c, mixAmount);
    float outA = mix(0.4, 1.0, lenv);
